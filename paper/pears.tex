...
% Pair selection section %
    ...
    The full pair sample is constructed from the set of all subhalos and their abundance matching realizations. Each pair will consist of a ``primary," the more massive object, and a ``secondary," which is the less massive companion. 
...
% \end{enumerate}
    ...
    Primary galaxies (equivalently, ``primary subhalos") are the most massive galaxy of their group, i.e., each group can have at most one primary galaxy.  
    ...
    This range was chosen to replicate a set of pairs with masses similar to LMC+SMC and MW-M31 mass pairs. The abundance matching prescription has a spread of $\sim 1$dex at $M_{h}\sim 10^{10}\Msun$, thus the set of selected primaries in each realization is unique. 

    A subhalo will not be selected as a primary if it is not the first most massive halo in the group, i.e., there is no more massive subhalo (by stellar mass) in the group. However, since our selection is based on stellar mass, and there is a large spread in the SMHM relation, the most massive object by stellar mass may not be the most massive subhalo in terms of subhalo mass. 
    This is by design, since our selection criteria are meant to roughly align with observational pair selection criteria which use the stellar mass ratio between the primary and companion to define a major or minor pair, rather than the DM halo mass. 
    
    Additional cases:
    \begin{itemize}
        \item In the case that a group hosts no galaxy in this stellar mass range in any realization, the group is removed from the sample. 
        \item If a subhalo passes the stellar mass cut for a primary, but does not have an associated companion from the selection criteria for a secondary (see next subsection), the subhalo will still be considered a primary. We refer to all primaries without companions as ``isolated primaries."
        \item If the most massive object by stellar mass is more or less massive than our 
    \end{itemize}
    ...
    The secondary companion galaxies are selected via their stellar mass ratio
    The second most massive galaxy by stellar mass in each group is then used 

    \begin{align*} 
    \mbox{\textbf{major pair:}}&\, \frac{1}{4}\leq \frac{\rm M_{*,2}}{\rm M_{*,1}}< 1 \\ 
    \mbox{\textbf{minor pair:}}&\, \frac{1}{10}\leq \frac{\rm M_{*,2}}{\rm M_{*,1}} < \frac{1}{4}\Msun
    \end{align*}
