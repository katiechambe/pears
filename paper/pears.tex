...
\begin{abstract}
  Galaxy pair fractions for low mass galaxies (DEFINE) are understudied across cosmic time. In particular, it is unclear whether low mass galaxy pair fractions are expected to evolve similarly to that of high mass galaxies (DEFINE high mass) in CDM theory. There has been no systematic study to ensure that any differences in CDM simulation can be captured in observational surveys. 
  In this study we *** Illustris, quick methods
    define pair fractions self-consistently, WE find BLAH
    we also a conduct a study to assess the impact of pair selection criteria based on separation cuts on the resulting pair fractions . We find BLAH. 
    If you apply the same separation, you wash out these effects, but we know that they exist in merger fracs, so how do we observe this ?    
    This sample is also used to place our Local Group of galaxy pairs in cosmological context across cosmic time for the first time --> high level 
\end{abstract}
...
% galaxy mergers 
    ...
    active galactic nuclei (AGN)~\citep[e.g.][]{Hopkins2008,comerford15,Blecha2018,ellison19}, 
    star formation~\citep[e.g.][]{Mihos1996, DiMatteo2008,Patton2013, hopkins13,patton20,hani20}, 
    and changes in morphology~\citep[e.g.][]{Conselice2003,lotz08,casteels14,rg17,Guzman-Ortega2023}. 
    Constraining the merger rate for galaxies is critical for quantifying the importance of mergers for galaxy evolution and hierarchical assembly.
    ...
    % it will be necessary to apply the proper theoretical framework for interpretting and analyzing low mass and high mass systems equivalently. 
    ...
    % We identify how the application of To this end, we apply one set of fixed separation cuts, and a second set of separation cuts that varies as a function of mass and redshift, and identify how these two methods 
...
% Pair selection section %
    ...
    \subsubsection{Selecting secondary companions}
        ...
        % where minor pairs are typically defined as having mass ratios between $1/4 > M_{*2}/M_{*1} > 1/10$ \citep{lotz11, rg15, Guzman-Ortega2023}.
...
\todo{redo ordering when finalized~}
...
% implications of difference and detectability 
...
In this paper, we construct a sample of isolated low mass and high mass pairs from the Illustris TNG100 simulation from $z=0-4.2$. % GB: define low mass and high mass. Also make it clear that pairs are chosen to belong in the same FoF group initially, ensuring they are isolated and not contaminated by projection effects. 
Major and minor pairs are determined using the stellar mass ratio between the primary and secondary halo, % GB : define major and minor
where stellar masses are assigned using an abundance matching prescription (Cite Moster) to generate 1000 separate realizations of all pairs at each redshift. % GB clarify that you sampled the error space.  
...
% GB: state goal -- our goal is to quantify the evolution of galaxy pairs across cosmic time and identify any differences in this evolution as a function of primary mass (high mass vs. low mass galaxies). 
...
\begin{itemize}
    ...
    virial radius of the FoF group (see Eqn). % GB: where teh virial radius can be estimated from the combined halo mass of the pair using SMHM relation. 
    %radius of the group halo.
    In particular, this separation cut selects between $\sim40-60\%$ of the full population of pairs at all redshifts, and recovers the differences in the redshift evolution of low mass vs. high mass pair fractions from $z=0-2.5$.   % GB: add here what Rvir corresponds to in kpc at specific redshift.   
    %the behavioral trend differences between low mass and high mass pairs from $z=0-2.5$ are distinct.
    \item The pair fractions of high and low mass pairs are 
    %somewhat i
    indistinguishable when a static physical separation cut is employed; i.e. a separation that does not evolve with redshift (e.g. $r_{sep}<50\kpc$), and is identical for low mass and high mass pairs. % GB: explain why this happens - with a fixed separation you are capturing more of the low mass pair sample than the high mass pair sample  \\
    \item  XXXX conclusion about local group analogs 
    % \item In order to recover 
\end{itemize}
...
Otherwise, selecting the same volume of two different mass scales will incorrectly bias inferred pair fractions, and wash out any differences between low and high mass pairs. % GB: connect the importance of recovering the differences between low and high mass pair fractions to efforts to translate pair fractions to merger rates using e.g. observability time scales that are fixed. Stress importance of avoiding static separation cuts in era of JWST where more low mass galaxies will be discovered. 


% GB i don't think you need much else beyond the above 

% Translating between pair fractions and merger rates for observationally defined samples requires an observability timescale. 
...
% Our results are consistent with prior works that have studied the pair fractions of low mass, physical galaxy pairs in the Illustris simulations, as well as those that adopt projected separation and velocity criteria over a similar stellar mass range as ours. 
% The agreement between the pair fraction trends from these projected studies, which have found good agreement with observations, and from our physical pair fraction study   to TNG's robust ability to reproduce the frequency of galaxy pairs from observations. 

% Talk about what we've missed and hinted at, and what our next project will kinda do to resolve this. 
