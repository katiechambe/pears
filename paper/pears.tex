...
\end{itemize}
Note that the subhalo that is assigned as primary or secondary may change between stellar mass realizations. 
...
\begin{table*}[]
  ...
  \begin{figure*}[htb]
    ...
    \caption{Median number of dwarf and massive primaries and pairs per abundance matching realization as a function of redshift, with shaded regions showing the $1^{\rm st}$ and 99$^{\rm th}$ percentiles. There are approximately 8 times as many dwarf primaries as massive primaries. There are fewer primaries and pairs at higher redshift, and the number of dwarf and massive primaries peaks around $z=1$. The evolution of the number of primaries is roughly similar between dwarf and massive galaxies, though the number of dwarf primaries drops steeply from $z=1$ to $z=0$. \textbf{However, the behaviour of the pair counts differs drastically between dwarf and massive pairs.} The number of dwarf pairs peaks around $z=2$, while the number of massive pairs increases nearly monotonically from $z=4-0$. Thus, the rate of dwarf pairs and massive pairs \textit{per primary} will evolve distinctly in the dwarf and massive pair case. 
      }
    \label{fig:counts}
  \end{figure*}
...
\begin{figure*}[htb]
  ...
  \caption{Median fraction (and 1-99 percentile shaded regions) of dwarf (green) or massive (pink) primaries with a major (left) or minor (right) companion. The fraction of massive primaries with a major or minor companion remains approximately constant for $z>1$, while at $z<\sim0.5$ the pair fraction increases sharply to $z=0$. However, the pair fraction for dwarf pairs evolves significantly differently to massive galaxy pairs. The fraction of dwarf primaries with a companion is approximately constant for $z>2$, but decreases monotonically from $z=2$ to $z=0$. The vast difference between the dwarf and massive pair fractions suggests \todo{XX - that pairs at different mass scales evolve differently ? Thus, can't just assume a massive pair fraction for a lower mass regime}. 
    }
  \label{fig:pairratio}
\end{figure*}
...
\begin{figure*}[htb]
  \centering
  \includegraphics[width=\textwidth]{separations.png}
  \caption{Median physical separation (in kpc) and 1-99 percentile shaded regions of 1000 abundance matching realizations as a function of redshift for dwarf pairs (left, green), and massive pairs (right, pink). Major pairs (high stellar mass ratio) are shown in solid lines, while minor pairs are depicted with dashed lines. Minor dwarf pairs tend to be $\sim10-20\kpc$ closer than major dwarf pairs, while minor massive pairs are between $10-50\kpc$ closer than major pairs. In general, as the redshift \textit{decreases} toward $z=0$, the average physical separation of a dwarf or massive pair \textit{increases}. The increase in low redshift median separations suggests that low separation pairs undergo mergers that remove them from the sample at later snapshots, thus increasing the median separation over time. The evolution of massive pair separations is approximately linear for $z>1$, increasing sharply for $z<0$, while the evolution of the dwarf pairs closely resembles a power law distrbution across the whole redshift range from $z=0-4$.
    }
  \label{fig:sep}
\end{figure*}
...
\begin{figure*}[htb]
  \centering
  \includegraphics[width=\textwidth]{separations_distributions.png}
  ...
  \todo{would it be instructive to make this plot, unnormalized, with z=0-3 on same plot to see if low sep pop numbers change drastically}
  \textbf{Takeaway from this plot?}. The number of widely separated pairs at low redshift is much greater than at high redshift, so these pairs either form late, or move apart from each other. Also note than many of the close pairs at high z may merge by $z=0$.
    }
  \label{fig:sep-dist}
\end{figure*}
...
\begin{figure*}[htb]
  \centering
  \includegraphics[width=\textwidth]{separations_scaled.png}
  ...
  \textbf{The difference between the scaled separations of dwarf and massive pairs highlights the mass dependence of the physical distribution of halo pairs, i.e., dwarf pairs are not simply a smaller scale version of massive pairs.}
    }
...
\begin{figure*}[htb]
  \centering
  \includegraphics[width=\textwidth]{velocities_distributions.png}
  \caption{
...
\begin{figure*}[htb]
  \centering
  \includegraphics[width=\textwidth]{velocities_scaled.png}
  \caption{Median scaled velocity (relative velocity of a pair divided by the virial velocity, defined as the circular velocity at the virial radius of the group \todo{need to remove the 2 in the virial radius calc!}) as a function of redshift. 
